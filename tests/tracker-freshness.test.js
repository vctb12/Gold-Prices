...
describe('tracker freshness module', () => {
  let mod;
  ...
  before(async () => {
    ...
    mod = await loadTrackerModule('freshness.js');
  });

  it('exports tracker badge constants', () => {
    assert.ok(mod.TRACKER_BADGE_CLASSES.includes('tracker-badge-live'));
    assert.equal(mod.SOURCE_BADGE_CLASS.live, 'tracker-source-badge--live');
    assert.equal(mod.STATUS_BADGE_CLASS.fallback, 'tracker-badge--stale');
  });

  it('builds a freshness model with badge metadata', () => {
    const freshness = mod.getFreshnessModel();
    assert.ok(['live', 'closed'].includes(freshness.effectiveKey));
    assert.equal(typeof freshness.tooltip, 'string');
    assert.ok(freshness.badgeClass.startsWith('tracker-badge'));
    assert.ok(freshness.sourceBadgeClass.startsWith('tracker-source-badge'));
  });

  it('applies status badge classes and accessible labels', () => {
    const node = createNode('span');
    node.className = 'tracker-badge--cached';
    const freshness = { badgeClass: 'tracker-badge-live', tooltip: 'Live source' };
    mod.applyStatusBadge(node, freshness, 'Live');
    assert.equal(node.textContent, 'Live');
    assert.equal(node.title, 'Live source');
    assert.equal(node.getAttribute('aria-label'), 'Live source');
    assert.ok(node.classList.contains('tracker-badge-live'));
  });

  it('builds source badges using safe DOM', () => {
    const badge = mod.buildSourceBadge({
      sourceBadgeClass: 'tracker-source-badge--live',
      tooltip: 'Provider',
      providerLabel: 'GoldPriceZ',
    });
    assert.equal(badge.tagName, 'SPAN');
    assert.match(badge.className, /tracker-source-badge--live/);
    assert.equal(badge.textContent, 'GoldPriceZ');
  });
});
