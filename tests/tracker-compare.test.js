...
describe('tracker compare module', () => {
  let mod;
  let comparisonCards;
  let comparisonEmpty;
  let state;
  ...
  before(async () => {
    installDom();
    comparisonCards = createNode('div');
    comparisonEmpty = createNode('p');
    state = {
      lang: 'en',
      compareCountries: ['AE'],
      compareKarats: ['24'],
    ...
    await loadCtx(
      state,
      {
        ...
        exportCompare2: createNode('button'),
      },
      () => 250,
      () => 2100
    );
    mod = await loadTrackerModule('compare.js');
  });

  it('renders comparison cards with partial DOM', () => {
    assert.doesNotThrow(() => mod.renderComparisonWorkspace());
    assert.equal(comparisonEmpty.hidden, true);
    assert.ok(comparisonCards.children.length > 0);
  });

  it('renders an empty comparison state with partial DOM', () => {
    state.compareCountries = [];
    assert.doesNotThrow(() => mod.renderComparisonWorkspace());
    assert.equal(comparisonEmpty.hidden, false);
  });
});
