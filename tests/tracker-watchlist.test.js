...
describe('tracker watchlist module', () => {
  let mod;
  let watchlistGrid;
  let state;
  ...
  before(async () => {
    ...
    mod = await loadTrackerModule('watchlist.js');
  });

  it('renders an empty watchlist state with partial DOM', () => {
    assert.doesNotThrow(() => mod.renderWatchlist());
    assert.match(watchlistGrid.textContent, /favorites|المفضلة|No/i);
  });

  it('renders favorite cards with partial DOM', () => {
    state.favorites = ['AED'];
    assert.doesNotThrow(() => mod.renderWatchlist());
    assert.ok(watchlistGrid.children.length > 0);
  });
});
