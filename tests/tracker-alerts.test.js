...
describe('tracker alerts module', () => {
  let mod;
  let alertList;
  let alertSummary;
  ...
  before(async () => {
    installDom();
    alertList = createNode('div');
    alertSummary = createNode('p');
    await loadCtx(
      {
        lang: 'en',
        alerts: [{ scope: 'spot', direction: 'above', target: 2000 }],
    ...
    mod = await loadTrackerModule('alerts.js');
  });

  it('renders alert summary with partial DOM', () => {
    assert.doesNotThrow(() => mod.renderAlertsSummary());
    assert.match(alertSummary.textContent, /1/);
  });

  it('renders alerts with partial DOM', () => {
    assert.doesNotThrow(() => mod.renderAlerts());
    assert.ok(alertList.children.length > 0);
  });
});
